...
ResuRank is a free, open-source desktop app that scores how well your resume matches a job description. Upload your resume PDF once, paste a job description, and get a 0–100% match score in seconds — combining semantic AI similarity with keyword overlap. All scoring runs locally; nothing leaves your computer. No subscription, no API key, no per-job limits. Customize term boosts to weight skills that matter to you, flag must-have keywords, filter out roles that don't fit your preferences, and exclude stopwords to clean up the match. Available for macOS, Windows, and Linux.
...
You can boost specific terms ("Rust", "Kubernetes") to weight skills you care about, flag must-have keywords so their absence actively reduces the score, and describe traits you *don't* want in a role so matching jobs are penalized accordingly. Exclude stopwords to remove noise, and inspect the full score breakdown to understand exactly how each component contributed.
...
- Preference mismatch penalty — describe traits you don't want in a role; jobs that match those traits are scored down proportionally
- Stopword exclusion — customize the word list ignored during scoring
- Score breakdown — see embedding, TF-IDF, overlap bonus, divergence penalty, missing keyword penalties, and preference mismatch penalty separately
