...
import {DEFAULT_MISSING_KEYWORD_SETTINGS, DEFAULT_PREFERENCE_MISMATCH_SETTINGS} from './storage/storage-adapter';
...
export class AppComponent implements OnInit {
  ...
  /**
   * Passed into the resume-picker outlet as the `onSwitched` input so it can
   * tell this component a different resume just became active. An arrow
   * class field rather than a method so the reference stays bound to `this`
   * when handed to a dynamically-created component instead of called
   * directly — see resume-picker-panel.token.ts.
   */
  reloadResume = (): void => {
    this.api.getResume().subscribe((r) => {
      this.resume.set(r);
